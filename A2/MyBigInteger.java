...
public class MyBigInteger  implements Comparable<MyBigInteger> {   //  We will cover what "implements Comparable" means later
	...
	public MyBigInteger dividedBy( MyBigInteger  divisor ) throws Exception{
		...
		MyBigInteger dividends = new MyBigInteger(this.base);
		int indexForDivision = this.coefficients.size() - divisor.coefficients.size();
		MyBigInteger divisorCopy = new MyBigInteger(this.base);
		for (int i = 0; i<divisor.coefficients.size();i++) {
			divisorCopy.coefficients.add(divisor.coefficients.get(i));
		}
		int thisQuotient;
		
		//throws an exception if the bases are different
		if(this.base != divisor.base) {
			throw new Exception ("Bases must be the same in the dividedBy method.");
		}
		//returns 0 if the divisor is greater than the dividend
		
	
		// if the first digit of the dividend is less than the divisor's, another digit must be included
		if((this.coefficients.get(this.coefficients.size()-1) <  divisor.coefficients.get(divisor.coefficients.size()-1)) && this.coefficients.size()!=1 && this.coefficients.size()!=1) {
			indexForDivision --;
		}
		//the dividend for the first division is made using the index for division 
		for(int i = indexForDivision; i < this.coefficients.size();i++ ) {
				dividends.coefficients.add(this.coefficients.get(i)); 
		}
		// if the divisor is greater than this, the quotient is returned
		...
		int counter = 0;
		while(true) {
			// if the divisor is bigger than the dividend and it isn't the first division, the, 0 is added to the quotient
			if(divisor.compareTo(dividends) > 0 && counter!=0) {
				thisQuotient = 0;
			
			}else{
				//otherwise, the quotient is set equal to the dividend/divisor coefficients
				thisQuotient = (dividends.slowdividedBy(divisor)).coefficients.get(0);
				//divisor is reset 
				divisor.coefficients = divisorCopy.coefficients;
			}
			// the obtained quotient is added to the MyBigInteger
			quotient.coefficients.add(0, thisQuotient);
			// the subtraction step is done using the timesSingleDigit and minus methods
			difference = dividends.minus(divisor.timesSingleDigit(thisQuotient));
	
			//the index for division is brought down by one
			indexForDivision --;
		
			// dividends are cleared
			dividends.coefficients.clear();
			//exits loop if it's done dividing
			if(indexForDivision<0) {
				break;
			}
			// the next digit of the dividend is brought down
			dividends.coefficients.add(this.coefficients.get(indexForDivision));
			//the difference coefficients are added to the dividend
			for(int i = 0; i<difference.coefficients.size();i++) {
				dividends.coefficients.add(difference.coefficients.get(i));
			}
			// removes leading 0's if needed
			while ((dividends.coefficients.size() > 1) && (dividends.coefficients.get(dividends.coefficients.size()-1) == 0)){
				dividends.coefficients.remove(dividends.coefficients.size()-1);
			}
			counter++;
		}
	...
	public MyBigInteger convert(int newBase) throws Exception {
		...
		MyBigInteger newBaseToDivide = new MyBigInteger(newBase, this.getBase());
    	MyBigInteger newDigit = new MyBigInteger(this.base);
		//   ADD YOUR CODE BELOW HERE
		
    	// if the bases are the same, the coefficients are copied to the result 
		if(this.base == newBase) {
			for(int i = 0; i < remainder.coefficients.size(); i++) {
				result.coefficients.add(remainder.coefficients.get(i));
			}
		
		// case where the new base is less than the current base
		}else if(this.base > newBase) {	
			//this loop adds the mod of the remaining number in the old base and the new base to divide by to the result until the remainder has no coefficients left
			while(remainder.coefficients.size()>=1)	{
				result.coefficients.add((remainder.clone()).mod(newBaseToDivide.clone()).coefficients.get(0));
				remainder = remainder.dividedBy(newBaseToDivide);
			}	
		// if none of these cases, that means the new base is greater than the old base
		}else {
			// the base to divide by must be converted into the old base
			newBaseToDivide = newBaseToDivide.convert(remainder.base);
			int convertedNewDigit=0;
			
			//this loop iteratesd until the dividend is less than the divisor
			while(remainder.compareTo(newBaseToDivide) != -1) {
				
				// the coefficidents of the new number is retrieved by using mod, and the remainder is divided
				newDigit.coefficients = remainder.mod(newBaseToDivide).coefficients;
				remainder = remainder.dividedBy(newBaseToDivide);
	
				//the new digit is converted by multiplying itself to its base to the power of its index in the coefficients
				for(int i = 0; i < newDigit.coefficients.size();i++) {
					convertedNewDigit+= (int) newDigit.coefficients.get(i)*Math.pow(this.base, i);
					
				}
				
				// the converted digit is added to the results and the converted and non-converted new digit variables are reset to 0
				result.coefficients.add(convertedNewDigit);
				convertedNewDigit = 0;
				newDigit.coefficients.clear();
				
			}
			// the following must be executed to find and add the last digit
			int lastDigit = 0;
			for(int i = 0; i < remainder.coefficients.size();i++) {
				lastDigit+= (int) remainder.coefficients.get(i)*Math.pow(this.base, i);
			}
			result.coefficients.add(lastDigit);
		}
		//   ADD YOUR CODE ABOVE HERE
	...
	public ArrayList<MyBigInteger>  primeFactors() throws Exception {
		...
		MyBigInteger rootM = new MyBigInteger(1, this.base);
		MyBigInteger m = this;
		
		//this loop finds the approximate square root of m
		while(rootM.times(rootM).compareTo(m) !=1) {
			rootM = rootM.times(two);
		}
		//the following few lines of code obtains a representation of roomM as an integer
		String rootMString = rootM.coefficients.toString();
		String newRootMString = "";
		for(int i = 0;i<rootMString.length();i++) {
			if(i!=0 && (i+2)%3 == 0) {
				newRootMString+=rootMString.charAt(i);
			}
		}
		int rootMInt = Integer.parseInt(newRootMString);
		
		//the loop iterates through integers between 2 and the approximate root of m 
		for(int i = 2; i< rootMInt; i++ ) {
			// the divisor and list of prime numbers are created
			MyBigInteger divisor = new MyBigInteger(i, this.base);
			ArrayList<MyBigInteger> listOfPrimes = primesToN(i,this.base);
			
			// if i appears in the last element of the list of prime numbers and m divides by it evenly, m is divided by it and i is added to the divisors
			if(listOfPrimes.get(listOfPrimes.size()-1).compareTo(divisor) == 0) {
					if((m.clone().mod(divisor.clone())).compareTo(zero) == 0) {
						m = m.clone().dividedBy(divisor);
						factors.add(divisor);
						// i is decremented so it stays as the same number to check if it works again
						i--;
					// at this point if the divisor is greater than m, the loop is broken
					} if (divisor.compareTo(m) == 1) {
						break;
					}
				}
		}
		//checks to see if there are factors greater than the square root
		MyBigInteger multiplier = new MyBigInteger(1, this.base);
			for(int i = 0; i<factors.size();i++) {
				multiplier = multiplier.times(factors.get(i));
			} 
			if(multiplier.compareTo(m) == -1) {
				factors.add(m.dividedBy(multiplier));
			}
